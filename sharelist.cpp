...
void ShareList::refreshFileItems()
{
    ...
    clearItems();
    // 如果文件列表不为空，显示文件列表
    if(m_shareFileList.isEmpty() == false)
    {
        int n = m_shareFileList.size();
        for(int i = 0;i < n;i++)
        {
...
void ShareList::getUserFilesList()
{
    ...
    // 按下载量降序127.0.0.1:8088/sharefiles?cmd=normal
    ...
    for(int i = 0;i < m_shareFileList.size();i++)
    {
        if(m_shareFileList.at(i)->item == item)
        {
            ...
            */
            ...
            //  -1: 下载的文件是否已经在下载队列中
            //  -2: 打开文件失败
            int res = downloadList->appendDownloadList(m_shareFileList.at(i), filePathName, true);
            if(res == -1)
            {
...
void ShareList::downloadFilesAction()
{
    ...
    // 看是否是共享文件下载任务，是才能往下执行, 如果不是，则中断程序
...
// 处理选中的文件
// cmd取值，Proterty属性，Cancel取消分享，Save转存文件
void ShareList::dealSelectedFile(ShareList::CMD cmd)
{
    ...
    for(int i = 0;i < m_shareFileList.size();i++)
    {
        if(m_shareFileList.at(i)->item == item)
        {
            ...
            {
                // 获取文件属性信息
                getFileProtery(m_shareFileList.at(i));
            }
            // 取消分享
            else if(cmd == Cancel)
            {
...
//==========>获取文件属性<==============
// 获取文件属性信息
void ShareList::getFileProtery(FileInfo* info)
{
...
void ShareList::cancelShareFile(FileInfo* info)
{
    ...
    {
        QMessageBox::warning(this, "操作失败", "此文件不是当前登录用户分享，无法取消分享！");
        return;
    }

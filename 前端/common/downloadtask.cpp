...
DownloadTask* DownloadTask::getInstance() // 保证唯一一个实例
{
    return instance;
}
...
int DownloadTask::appendDownloadList(FileInfo *info, QString filePathName, bool isShare)
{
    ...
    // 获取布局
    DownloadLayout* pDownload = DownloadLayout::getInstance();
    QVBoxLayout* layout = (QVBoxLayout*)pDownload->getDownloadLayout();
...
bool DownloadTask::isEmpty()         // 判断下载队列是否为空
{
    return list.isEmpty();
}

bool DownloadTask::isDownload()      // 判断是否有文件正在下载
{
    //遍历队列
    for(int i = 0;i < list.size();i++)
    {
        if(list.at(i)->isDownload) return true;
    }
    return false;
}

bool DownloadTask::isShareTask()     //第一个任务是否为共享文件的任务
{

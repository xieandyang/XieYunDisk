...
UploadTask* UploadTask::getInstance() // 保证唯一一个实例
{
    return instance;
}
...
bool UploadTask::isEmpty()         // 判断上传队列是否为空
{
    return list.isEmpty();
}

bool UploadTask::isUpload()        // 判断是否有文件正在上传
{
    // 遍历队列
    for(int i = 0;i < list.size();i++)
    {
        if(list.at(i)->isUpload == true) return true;
    }
    return false;
}

// 取出第1个上传任务，如果任务队列没有任务在上传，设置第一个任务上传

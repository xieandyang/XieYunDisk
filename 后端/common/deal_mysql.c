...
{
    fprintf(stderr, "%s:\nError %u (%s) \n", title, mysql_errno(conn), mysql_errno(conn));
}
...
MYSQL* mysql_conn(char* user_name, char* passwd, char* db_name)
{
    ...
    // 用来分配或者初始化一个MYSQL对象， 用于连接mysql服务器
    ...
    //passwd: 参数包含用户的密码
    if(mysql_real_connect(conn, "47.113.193.186", user_name, passwd, db_name, 3306, NULL, 0) == NULL)
    {
...
END:
    if(res_set != NULL)
    {
        // 完成所有对数据的操作后，调用mysql_free_result释放

 ...
 */
/* -------------------------------------------*/
int recv_save_file(long len, char* user, char* filename, char* md5, long* p_pos, long* p_size)
{
    int ret = 0;
    ...
    //===========> 开始处理前端发送过来的post数据格式 <============
    char* begin = file_buf; // 内存起点
    char* p = begin;
    /*
       ------WebKitFormBoundary88asdgewtgewx\r\n
       Content-Disposition: form-data; user="mike"; filename="xxx.jpg"; md5="xxxx"; pos=0; size=10240\r\n
    ...
    //========================================获取文件名字
    //"; filename="xxx.jpg"; md5="xxxx"; pos=0; size=10240\r\n
    //   ↑
    begin = k;
    ...
    k = strchr(q, ';');
    char pos[256] = {0};
    strncpy(pos, q, k-q);   //内容
    pos[k-q] = '\0';

    *p_pos = strtol(pos, NULL, 10); //字符串转long
    ...
    *p_size = strtol(tmp, NULL, 10); //字符串转long
    ...
    LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PROC, "pos = %d\n", *p_pos);
...
int main()
{
    ...
    while(FCGI_Accept() >= 0)
    {
        LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PROC, "正在接受中\n");
        ...
        else len = strtol(contentlength, NULL, 10); // 字符串转long， 或者atol
        // 2、根据长度将post数据块读到内存
        if(len <= 0) // 没有信息
        {
        ...
        {
            //===============> 得到上传文件  <============
            if(recv_save_file(len, user, filename, md5, &pos, &size) < 0)
            {
                ret = -1;
                LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PROC, "recv_save_file error\n");
                goto END;
            }
            LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PROC, "%s成功上传[%s, 大小：%ld, md5码: %s]到本地\n", user, filename, size, md5);

            if(pos + 1024 * 100 >= size)
            {
                //===============> 将该文件存入fastDFS中,并得到文件的file_id <============
                if(upload_to_dstorage(filename, fileid) < 0)
                {
                    ret = -1;
                    LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PROC, "upload_to_dstorage error\n");
                    goto END;
                }
                ...
                {
                    ret = -1;
                    LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PROC, "make_file_url error\n");
                    goto END;
                }
                ...
                {
                    ret = -1;
                    LOG(UPLOAD_LOG_MODULE, UPLOAD_LOG_PROC, "store_fileinfo_to_mysql error\n");
                    goto END;
                }
            }
